...
FCollisionDetector::FSweptCylinder FCollisionDetector::CreateSweptCylinder
(const ICollisionShape& Shape, const FTransform& StartTransform, const FTransform& EndTransform)
{
	using SweptCylinder = FCollisionDetector::FSweptCylinder;

	SweptCylinder result;

	// 시작점과 끝점 설정
	result.Start = StartTransform.Position;
	result.End = EndTransform.Position;

	// 이동 벡터 계산
	Vector3 displacement = result.End - result.Start;
	result.Length = displacement.Length();

	// 축 방향 계산 (정규화)
	if (result.Length > KINDA_SMALL) {
		result.Axis = displacement / result.Length;
	}
	else {
		result.Axis = Vector3::Up; // 기본값
		result.Length = 0.0f;
	}

	// 회전을 고려한 시작점과 끝점에서의 경계 구 반지름 계산
	float startRadius = CalculateBoundingSphereRadius(Shape, StartTransform);
	float endRadius = CalculateBoundingSphereRadius(Shape, EndTransform);
...
float FCollisionDetector::CalculateBoundingSphereRadius(const ICollisionShape& Shape, const FTransform& Transform)
{
	if (Shape.GetType() == ECollisionShapeType::Sphere)
	{
		// 구체는 회전에 영향 받지 않음
		return Shape.GetScaledHalfExtent().x;
	}
	else //나머지는 직육면체로 근사
	{
		// 박스의 회전을 고려한 경계 구 계산
		Vector3 Extent = Shape.GetScaledHalfExtent();

		// 회전 행렬 얻기
		Matrix RotMatrix = Transform.GetRotationMatrix();

		// 회전된 각 모서리 점들 중 가장 먼 거리 찾기
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			// 회전 적용
			XMVECTOR RotatedCorner = XMVector3Transform(XMLoadFloat3(&Corner), RotMatrix);

			// 거리 계산
			float DistSq = XMVectorGetX(XMVector3LengthSq(RotatedCorner));
